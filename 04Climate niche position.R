...
GMPD_Kernel_Data <- kd_prep(clim.raw = BIO_050612,
                            spat.dat = GBIF_Data,
                            samp.dat = GMPD_Distances_Data, # doesn't matter that I'm not filtering because of rasterisation
                            pca.full = PCA_Full,
...
GMPD_Climate_Data <- GMPD_Kernel_Data_half$samples.out %>% 
  ...
  full_join(GMPD_Climate_Data, 
            by = names(.)[!grepl("eddist|angle|density", names(.), ignore.case = TRUE)],
            suffix = c("_5", ""))
