####################################################################################################
##### FUNCTIONS FOR CALCULATING POSITION OF PARASITE SAMPLE LOCATIONS IN GEOGRAPHIC SPACE AND RANGE TRAITS ######################
...
# ## IUCN poly method
# para.dat <- GMPD_Data_res_all
# range.pol <- IUCN_Native_Data
# method <- "iucn"
# tax <- "sub"

# ## GBIF point method
# para.dat <- GMPD_Data_cln_all
# range.dat <- GBIF_Data
# method <- "gbif"
# tax <- "sub"

range_distances <- function(para.dat, range.pol, range.dat, method=c("gbif","iucn"), tax=c("species", "sub")){
  if(method == "iucn"){
    ## Restrict range data to subgroups
    if(tax == "sub") {
      range.pol@data <- filter(range.pol@data, subgroup != "not used") # restrict range.dat to just data on subgroups
      para.dat <- filter(para.dat, subgroup != "not used") # restrict para.dat to just data on subgroups
      
      range.pol@data$binomial <- paste(range.pol@data$binomial , range.pol@data$subgroup)  # update 'species' column to contain the subgroup epithets
      para.dat$HostCorrectedName <- paste(para.dat$HostCorrectedName, para.dat$subgroup)
    }
    
    range.pol <- range.pol[range.pol@data$binomial %in% unique(para.dat$HostCorrectedName), ] # restrict range.pol to match hosts in dat
    
    dat.sets <- split(para.dat, f = para.dat$HostCorrectedName)
    ...
    dat.out <- lapply(dat.sets, function(k){
      ...
      if(nrow(range.pol.sub)>0) {
        ...
        range.traits <- data.frame("HostCorrectedName" = unique(k$HostCorrectedName),
                                   ...
                                   "RangeArea" = NA,
                                   "RangeMedian" = NA)
        k$EquatorwardsDist <- k$EquatorwardsProp <- k$MedianDist <- k$MedianProp <- NA
    ...
    ## Restrict range data to subgroups
    if(tax == "sub") {
      range.dat <- filter(range.dat, subgroup != "not used") # restrict range.dat to just data on subgroups
      para.dat <- filter(para.dat, subgroup != "not used") # restrict para.dat to just data on subgroups
      
      range.dat$species <- paste(range.dat$species, range.dat$subgroup)  # update 'species' column to contain the subgroup epithets
      para.dat$HostCorrectedName <- paste(para.dat$HostCorrectedName, para.dat$subgroup)
    }
    
    ## Continue calculating distances
    range.dat <- filter(range.dat, species %in% unique(para.dat$HostCorrectedName)) # restrict range.dat to match hosts in dat
    
    hosts.noGMPD <- unique(filter(para.dat, !(HostCorrectedName %in% unique(range.dat$species)))$HostCorrectedName) ## ID any mammal species without data in parasite database
    print(c("Hosts in parasite database without range data are: ", hosts.noGMPD))
    
    para.dat <- filter(para.dat, HostCorrectedName %in% unique(range.dat$species))
    dat.sets <- split(para.dat, f = para.dat$HostCorrectedName) # split input data by host
    
    dat.out <- lapply(dat.sets, function(k){
      # k <- dat.sets[[1]]
      range.dat.sub <- range.dat[range.dat$species == unique(k$HostCorrectedName), ] # subset range.dat to current host
      coordinates(range.dat.sub) <- ~ lon + lat
      range.abs.vals <- abs(range.dat.sub@coords[,"lat"]) # vector of absolute latitudes in gbif
      
      range.max <- max(range.abs.vals)
      range.min <- min(range.abs.vals)
      range.span <- geosphere::distGeo(c(0, range.max), c(0, range.min)) # in m
      ...
      k$EquatorwardsDist <- geosphere::distGeo(k[,c("zeros","Latitude")], c(0,range.min)) # vertical distances from parasite sample locations to lowest latitude of host range (in m)
      k$EquatorwardsProp <- k$EquatorwardsDist/range.span # as a proportion of range span
      k$MedianDist <- geosphere::distGeo(k[,c("zeros","Latitude")], c(0,range.median)) # vertical distances from parasite sample locations to median of host range (in m)
